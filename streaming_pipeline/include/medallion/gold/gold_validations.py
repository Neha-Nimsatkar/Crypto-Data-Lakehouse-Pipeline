...
silver_latest = spark.read.format("delta").load(silver_path) \
                  .filter("coin_id = 'bitcoin'") \
                  .orderBy(f.col("event_timestamp").desc()) \
                  .select("price_usd").first()[0]
...
# --- Check 1: Snapshot Uniqueness ---
snap_count = df_snapshot.groupBy("coin_id").count().filter("count > 1").count()
if snap_count > 0:
    print(f"❌ INTEGRITY ERROR: Snapshot has duplicate entries for {snap_count} coins!")
else:
...
btc_anomalies = df_trends.filter(
    ...
    ((f.col("daily_avg_price") < 20000) | (f.col("daily_avg_price") > 150000))
).count()
if btc_anomalies > 0:
...
# --- Check 4: Current Market Snapshot ---
